...
def _adcsphereGPA(mripar,tpar):
	...
	L_um = tpar[0]                 # Cell diameter L in um
	R_um = L_um/2.0                # Cell radius R = L/2.0 in um
...
def _sigsphereGPA(mripar,tpar):
	...
	adcval = _adcsphereGPA(mripar,tpar)   # tpar = [L,D0] with L -> cell diameter L in um, D0 -> intrinsic cytosol diffusivity D0 in um2/ms
...
def _sigDinDexTD(acqpar,tispar):
	...
	mrisig = fval*_sigsphereGPA(acqpar,np.array([lval,d0val])) +  ( 1 - fval )*np.exp(-(mybvals/1000.0)*dex)
...
def _sigDin(acqpar,tispar):
	...
	# Synthesise MRI signal
	mrisig = _sigsphereGPA(acqpar,np.array([lval,d0val]))
	mrisig[mybvals==0] = 1.0
	mrisig = S0sig*mrisig
...
if __name__ == "__main__":
	...
	parser.add_argument('--pmin', metavar='<list>', help='list or array storing the lower bounds for tissue parameters. These are: L,D0,S0 for model "Din";  L,D0,F,Dexinf,S0 for model "DinDex"; L,D0,F,Dexinf,Beta,S0 for model "DinDexTD". The symbols stand for: L -> cell size (diameter), in (um); D0 -> intrinsic intra-cell diffusivity in (um2/ms);  F -> intra-cellular signal fraction; Dexinf -> long-time extra-cellular apparent diffusion coefficient, in (um2/ms); Beta -> extra-cellular diffusion-time dependence coefficient in (um2); S0 -> non-DW signal level, with respect to the maximum measured signal (parametrisation: S(b = 0) = S0*max(S)).. Note that the extra-cellular apparent diffusion coefficient is written as Dex = Dexinf + Beta/t, where t is the gradient separation. Default: "8.0,0.8,0.01" for model "Din"; "8.0,0.8,0.01,0.01,0.6" for model "DinDex"; "8.0,0.8,0.01,0.01,0.01,0.6" for model "DinDexTD". For more information on the models, please look at input parameter modstr below.')
	parser.add_argument('--pmax', metavar='<list>', help='list or array storing the lower bounds for tissue parameters. These are: L,D0,S0 for model "Din";  L,D0,F,Dexinf,S0 for model "DinDex"; L,D0,F,Dexinf,Beta,S0 for model "DinDexTD". The symbols stand for: L -> cell size (diameter), in (um); D0 -> intrinsic intra-cell diffusivity in (um2/ms);  F -> intra-cellular signal fraction; Dexinf -> long-time extra-cellular apparent diffusion coefficient, in (um2/ms); Beta -> extra-cellular diffusion-time dependence coefficient in (um2); S0 -> non-DW signal level, with respect to the maximum measured signal (parametrisation: S(b = 0) = S0*max(S)).. Note that the extra-cellular apparent diffusion coefficient is written as Dex = Dexinf + Beta/t, where t is the gradient separation. Default: "40.0,3.0,1.4" for model "Din"; "40.0,3.0,1.0,3.0,1.4" for model "DinDex"; "40.0,3.0,1.0,3.0,10.0,1.4" for model "DinDexTD". For more information on the models, please look at input parameter modstr below.')
	...
	parser.add_argument('--modstr', metavar='<string>', default='DinDex', help='string specifying the signal model to fit. Choose among: Din (extra-vascular signal dominated by intra-cellular diffusion), DinDex (extra-vascular signal features both intra-cellular and extra-cellular contributions, without diffusion time dependence in the extra-cellular ADC), DinDexTD (extra-vascular signal features both intra-cellular and extra-cellular contributions, with diffusion time dependence in the extra-cellular ADC). Default: DinDex. Intra-cellular diffusion is modelled using the Gaussian Phase Approximation (GPA) formula for diffusion within sphere of Balinov et al. "The NMR self-diffusion method applied to restricted diffusion. Simulation of echo attenuation from molecules in spheres and between planes." Journal of Magnetic Resonance, Series A 104.1 (1993): 17-25, doi: 10.1006/jmra.1993.1184')
