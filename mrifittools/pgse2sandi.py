...
def run(mrifile, mriseq, output, maskfile=None, noisefile=None, Nword=5, regnorm=2, regw=0.001, pmin=[0.5,2.99,0.5,1.0,0.01,0.01,0.95], pmax=[3.0,3.0,3.0,15.0,0.99,0.99,1.05], nthread=1, slicedim=2, fitting='trust-constr', saveSM=False):
	...
	fin_obj = nib.Nifti1Image((1.0 - Fexmap)*Finmap*k_data,m_obj.affine,buffer_header)
	nib.save(fin_obj, '{}_Fin_voxel.nii'.format(output))

	fex_obj = nib.Nifti1Image(Fexmap,m_obj.affine,buffer_header)
	nib.save(fex_obj, '{}_Fex_voxel.nii'.format(output))

	fsoma_obj = nib.Nifti1Image((1.0 - Fexmap)*(1.0 - Finmap)*k_data,m_obj.affine,buffer_header)
	nib.save(fsoma_obj, '{}_Fsoma_voxel.nii'.format(output))
...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description='Regularised non-linear least square fitting of the SANDI model from Palombo et al, NeuroImage 2020, doi: 10.1016/j.neuroimage.2020.116835. Third-party dependencies: nibabel, numpy, scipy. Last successful test with nibabel=5.1.0, scipy=1.10.1, numpy=1.23.5. Francesco Grussu, Vall d Hebron Institute of Oncology <fgrussu@vhio.net>. Copyright (c) 2024, Vall d Hebron Institute of Oncology (VHIO), Barcelona, Spain. All rights reserved.')
