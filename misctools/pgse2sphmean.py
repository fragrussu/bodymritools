...
def run(mrifile, mriseq, output): 
	...
	### Done
	timefinal = time.time()
	print('    ... done - spherical mean computed in {} sec'.format(timefinal - timeinitial))
	print('')
...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description='Calculation of the spherical mean signal for a Pulsed-Gradient Spin Echo (PGSE) experiment. The script is intended for use on a data acquired with PGSE where multiple gradient directions are used to acquire images at varying (b,δ,Δ). The script will output the mean of all volumes acquired at fixed (b,δ,Δ). Author: Francesco Grussu, Vall d Hebron Institute of Oncology <fgrussu@vhio.net>. Copyright (c) 2026, Fundació Privada Institut d Investigació Oncològica de Vall d Hebron (Vall d Hebron Institute of Oncology, VHIO, Barcelona, Spain)')
	...
	parser.add_argument('out', help='output: root file name of output files; output NIFTIs will be stored as double-precision floating point images (FLOAT64), and the file names will end in: *_SphMean.nii.gz: 4D NIFTI file storing a series of spherical mean signals at fixed b, grad. dur, grad. separation *_SphMean.acq.txt: space-separated text file storing the sequence parameters correspnding to *_SphMean.nii.gz. It features the same number of columns as *_SphMean.nii, and has 3 lines: -- First row: b-values in s/mm2  -- Second row: gradient duration small delta (δ) in ms -- Third row: gradient separation Large Delta (Δ) in ms.')
